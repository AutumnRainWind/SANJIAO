QUnit.test("equilateral have three equal sides", function( assert ) {
	...
	assert.equal(triangle.isEquilateral(2,2,1),false,"2,2,1 is not equilateral") ;
...
QUnit.test("isosceles triangle has two equal sides", function( assert ) {
	...
	assert.equal(triangle.isIsosceles(0,0,1),false,"5,5,2 is not isosceles") ;
	assert.equal(triangle.isIsosceles(-1,-1,5),false,"5,5,2 is not isosceles") ;
});
